...
脚本自动完成：Python 环境检查 → Hermes Agent → Trade 安装 → 32 个 skills → 数据库初始化。
...
| 新客引导 | 首次使用引导：创建公司→配置 Agent 身份→初始化文档库 |
...
├── skill_registry.py      32 个 skill 注册表（纯数据）
├── bootstrap.py            启动引导（Hermes 版本检查、env 加载、Skills 同步）
├── app.py                  FastAPI app factory
└── ... + 32 个业务模块

skills/                    32 个 B2B skills（Markdown 驱动）
