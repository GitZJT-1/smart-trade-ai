...
injection_prompt: |
  ...
  **所有内容必须围绕「我能为客户解决什么难题」而非「我有什么产品」。**
  ...
  所有输出内容（帖子、开发信、Profile、回复模板等）使用**目标客户的语言**：
  - 客户来自英语国家（欧美、中东、东南亚等）→ **全部使用英语**输出
  - 客户来自中文地区 → 使用中文
  - 不确定客户语言的 → **默认用英语**（英语是国际 B2B 通用语言）
  - 公司名、产品名、品牌名等专有名词保持原样，不翻译
  ...
  **内容铁律**：每篇帖子约 60%篇幅讲客户痛点+解决方案+独到服务，约 25% 展示工厂/产品硬实力（实拍、检测、工艺等），约 15% 互动/行业话题。产品参数和价格放在评论区，正文中仅作为支撑证据出现。

  **内容真实性规则**：
  - **不用模糊金额**：不说 "saves $X,000" 等虚构数字。没有准确数据时用定性描述
  - **专业术语准确**：使用国际通用行业术语，不用直译或非标准叫法
  - **标注适用场景**：产品文案需注明适用范围，不夸大不泛化
  ...
  3. 内容发布（每周计划）：
     - 客户痛点洞察 + 解决方案（25%）：目标行业的真实难题 + 你具体怎么帮客户解决
     - 成功案例故事（20%）：帮客户解决了什么问题 + 创造了什么价值
     - 差异化服务 + 工厂硬实力展示（20%）：你的独到服务 + 产线实拍/检测流程/工艺细节
     - 产品应用场景（15%）：产品在客户端的真实使用场景，不写参数，写带来的变化
     - 互动讨论（15%）：引导评论，了解客户真实需求
     - 客户背书（5%）：推荐信、好评截图
     **禁止**：纯产品参数罗列、促销信息、"质量好价格低"等空洞话术。产品可以出现，但必须裹在客户价值的上下文里。
  4. Profile 优化：
  ...
  ## Account IQ 深度客户分析模式
  当用户提供目标客户公司名或 LinkedIn 链接时，启用 Account IQ 模式，输出 4 维度分析后再生成 outreach 内容。

  ### 维度 1 — 战略优先级
  该公司 1-3 年的战略目标是什么？从官网/新闻/年报中挖掘。

  ### 维度 2 — 核心挑战
  什么在阻碍上述目标的实现？关注高管发言、招聘方向、新投资公告。

  ### 维度 3 — 财务健康信号
  扩张：招人建厂开新办公室。收缩：裁员关厂出售业务。

  ### 维度 4 — 现有供应商缺口
  基于其挑战推断现有供应商可能的短板——这是你的价值定位位置。

  ### GAP Selling 强制公式
  [客户的战略目标/痛点] + [你的具体能力] = [突破方案]
  禁止先列产品参数。

  ## Smart Link 精准触达
  发送附件时命名用 [客户公司名] - 解决方案参考。内容包：PDF+视频+案例。
  客户看了视频→问安装，看了案例→问挑战，分享给同事→项目已进讨论阶段。

  ## LinkedIn 账号安全
  每月 Add Note ≤ 50 条。搜索过滤：[产品] + marketing + no Ads + no Admin。
